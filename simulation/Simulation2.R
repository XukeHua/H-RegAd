...
as_num_mat <- function(M, nm) {
  ...
  if (!is.matrix(M)) stop(nm, " Not a matrix")
  M
}
...
plot_true_vs_pred_index <- function(beta_true, beta_hat,
                                    main = "H-ADMM (with intercept): True vs Pred on index axis",
...
for (ii in 1:ngrp) C1[ii, (tind[ii] + 1):tind[ii + 1]] <- 1
C_raw <- matrix(0, ngrp, p)
C_raw[, 1:ncol(C1)] <- C1
C_sp <- t(C_raw)
...
if (is.null(fit_smoke)) {
  stop("Smoke test failed: the original error has already been printed above,
       please fix it according to the original error")
}
...
if (is.null(best)) {
  stop("None of the (tau, lambda) combinations produced usable results. 
       It's recommended to first relax tol_admm, reduce rho, or just 
       check the stop_reason from the smoke test.")
}
